...
namespace Agile.NET_Deobfuscator_Updated.Modules
{
    ...
    {
        public override string ModuleName() => "String Decryption";
        private byte[] data;
            ...
            foreach(var module in context.Assembly.Modules)
            {
                ...
                foreach(var type in module.GetAllTypes())
                {
                    ...
                    foreach(var method in type.Methods.Where(x => x.HasMethodBody))
                    {
                        ...
                        {
                            if(method.CilMethodBody.Instructions[i].OpCode == CilOpCodes.Ldstr && method.CilMethodBody.Instructions[i+1].OpCode== CilOpCodes.Call && method.CilMethodBody.Instructions[i+1].Operand is MethodDefinition decryptionMethod)
                            {
        ...
        {                                                              
            ...
            {
                stringBuilder.Append(Convert.ToChar((int)(A_0[i] ^ (char)data[i % data.Length])));
            }
